...
export class HightlightSearchEngine {
    constructor(wrapper = null) {
        ...
        StateManager.subscribe('readerSection', (el) => this.scope = el)
    }
    ...
    searchPage() {
        ...
        const range = appState.selectionRange //|| selection.getRangeAt(0);
        const selectedText = appState.selectedText //|| selection.toString();

        // reset hightlight first
        this.removeHighlightedSpans()

        // store screen coordinates
        this.screenXY(range);

        // No search for more than one word
        if (!selectedText || selectedText === " " || selectedText.split(" ").length > 1 || selectedText.length === 1) return;
...
export const hightlightsearch = new HightlightSearchEngine();
