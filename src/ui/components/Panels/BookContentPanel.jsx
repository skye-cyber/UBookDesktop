...
export const BookContentPanel = ({ }) => {
    ...
    return (
        <div
            ref={panel}
            id="sidepane"
            className="fixed z-40 left-0 w-[300px] bg-gradient-to-br from-indigo-900 to-indigo-950 dark:from-[#001f2b] dark:to-[#001f2b] h-[90dvh] sm:h-[93vh] shadow-2xl transform transition-transform -translate-x-full xl:translate-x-0 transition-colors duration-700 ease-in-out select-none">
            ...
            <section className="h-fit max-h-[72%] overflow-y-auto scrollbar-custom select-none bg-indigo-950/25 dark:bg-[#002b36] rounded-lg">
                <ul className="mt-3">
                    ...
                    <div onClick={() => document.dispatchEvent(new CustomEvent('show-notes'))} className='group flex items-center p-2 text-white dark:text-gray-300 hover:bg-indigo-800/80 dark:hover:bg-[#00445f]/70 cursor-pointer'>
                        <li id="notes" className="transiton-all duration-300  group-hover:translate-x-2 ease-in-out">📔 Notes</li>
                    </div>
                </ul>
            </section>
            <div className="absolute left-0 bottom-16 flex items-center justify-center w-full px-0">
